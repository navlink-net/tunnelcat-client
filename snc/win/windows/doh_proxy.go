...
// DoHProxy is a minimal local DNS-over-HTTPS proxy.
//
// It listens for UDP DNS queries on 127.0.0.1:53 and forwards each one as an
// RFC 8484 HTTPS POST to 1.1.1.1/dns-query.  The HTTPS request goes out
// through the TUN adapter (the 1.1.1.1 bypass route must NOT be present), so
// the query travels encrypted through the SOCKS5/tunnel stack to the exit node
// and on to Cloudflare.
//
// Used as fallback when "netsh dns add encryption" is unavailable (Windows 10
// versions older than KB5033375 / before 21H2).
//
// Lifecycle:
//   proxy, err := newDoHProxy()
//   proxy.start()
//   ... (tunnel is up, TUN DNS set to 127.0.0.1)
//   proxy.stop()
...
const (
	...
	// The DialContext in newDoHProxy hardcodes 1.1.1.1:443 so no DNS lookup is needed.
...
func (p *DoHProxy) start() error {
	...
	if err != nil {
		return fmt.Errorf("doh-proxy: bind %s: %w (need admin or port not in use)", dohProxyListenAddr, err)
	}
	p.conn = conn
	p.wg.Add(1)
	go p.serve()
